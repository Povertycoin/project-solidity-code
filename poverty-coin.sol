...
//PovertyCoin
/////////////////////////////////////////
contract PovertyCoin is BaseInterface {
    ...
    //Addresses (testnet)
    address public constant _adminAddress      = address(0x72290A428a915c6151867ACE8adc5b970B7A2AC8); 
    address public constant _charityAddress    = address(0x6075373760A81dE3f022C0C4390938565b14819d); 
    address public constant _devAddress        = address(0x9B9b1B71C490Da5Dc0Ae54675C318C47608cA1c9);
    address public constant _marketingAddress  = address(0x71507918525cbfAf45F69dc1Fa510040a81876EC);
    address public constant _liquidityAddress  = address(0x5AD388b8EaC05791893606F328e753deC95347A2); 
    address public constant _reflectionAddress = address(0x4C8027f250a30CF6F802C0F0d4279414E33beb82); 
    address public constant _burnerAddress     = address(0x7ecf8A0615c4B7b6BF7D2c50028C9737B63b1b85); 

    address public constant _andreiAddress     = address(0x18785AEE023b7b3FB14e603724D0Db89b8edb27a);
    address public constant _claudiaAddress    = address(0x294A419CbC68dAa43A45e7ebf57581D99ef7Bfbd);
    address public constant _patrickAddress    = address(0x1a50Db0Cef064F8d3F6FaCb0bfD946E7B46340Cf); 
    ...
    uint256 private constant _stakingRewardRateMax = 100; // 100 - meaning 10%
    ...
    uint256 private constant _stakingBonusRate = 1; // 0.1%
    uint256 private constant _stakingBonusPerConnections = 10;
    uint256 private constant _stakingBonusMaxConnections = 150;
    ...
    constructor() {
        ...
        //1.25% to the marketing address
        _balances[_marketingAddress]  = _balances[address(this)].div(80); // 100% / 80 = 1.25%
        //1.25% to the dev address
        _balances[_devAddress]  = _balances[address(this)].div(80); // 100% / 80 = 1.25%

        //take out 12.5%
        _balances[address(this)] = _balances[address(this)].sub(_balances[address(this)].div(8)); //take 12.5% out 
        ...
        /////////////////////////////////////////
        //Exclude liquidity & charity addresses from fees
        //team, marketing, dev address have to pay fees 
        //admin, burn, redistribution addresses cannot perform transfers
        /////////////////////////////////////////
    ...
    receive() external payable addressCheck
    {
        ...
        uint256 coinsAmount = msg.value.mul(_sellingCampaignOneBNBPriceInCoins); //in wei
        ...
        _balances[_liquidityAddress] = _balances[_liquidityAddress].add(coinsAmount.mul(_sellingCampaignLiquidityCoef).div(10));

        //subtract liquidity coins from the contract
        _balances[address(this)] = _balances[address(this)].sub(coinsAmount.mul(_sellingCampaignLiquidityCoef).div(10));
    ...
    function _transfer(address sender, address recipient, uint256 amount) private addressCheck {
    ...
    function stakeCoins(uint256 amount) public {
        //addresses not allowed to use this function
        if (msg.sender == _adminAddress) return;
        if (msg.sender == _liquidityAddress) return;
        if (msg.sender == _charityAddress) return;
        ...
        _stakingBalances[msg.sender] = StakeEntry(amount, block.timestamp + 7 days, totalRewardRate);
        emit Stake(msg.sender, amount, totalRewardRate, block.timestamp + 7 days);
    }
